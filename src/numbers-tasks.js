...
function getLinearEquationRoot(a, b) {
  if (a === 0) {
    return 'It is not a liner equation';
  }
  return -b / a;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const sсalar = x1 * x2 + y1 * y2;
  const aLength = Math.sqrt(x1 ** 2 + y1 ** 2);
  const bLength = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosAngel = sсalar / (aLength * bLength);
  const arccos = Math.acos(cosAngel);
  return arccos;
}
